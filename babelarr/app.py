...
class Application:
    ...
    def handle_new_mkv(self, path: Path) -> None:
        """Queue a freshly detected MKV for tagging once it stabilizes."""
    ...
    def request_mkv_scan(self, path: Path | None = None) -> None:
        """Signal the MKV scanner thread to perform a scan.

        When *path* is provided, only that MKV file is processed. Otherwise a
        full directory walk is scheduled.
        """
    ...
    def mkv_scan_worker(self) -> None:
        """Background worker that processes MKV scan requests."""
    ...
    def mkv_scan(self) -> None:
        """Run the MKV scanner once."""

        if not self._mkv_scanner:
            return
        logger.info("mkv_scan_start")
        files, tagged = self._mkv_scanner.scan()
        logger.info("mkv_scan_complete files=%d tagged=%d", files, tagged)
    ...
    def mkv_scan_file(self, path: Path) -> None:
        ...
        files, tagged = self._mkv_scanner.scan_files([path])
        logger.info(
            "mkv_scan_file_complete path=%s processed=%d tagged=%d",
            path,
            files,
            tagged,
        )
